<?php
    ...
    $num_registered += $num_paid;
        $query = "SELECT count(*) FROM data WHERE Shirtreceived='Complete'";
    $result = mysqli_query($con, $query);
    $row = mysqli_fetch_array($result);
    $num_cd = $row['count(*)'];
	...
	<hr style="color: white; border:1px dotted white;">
		<form action="check_member_details_shirt.php" name="newregistration" onsubmit="return(validate());" method="post" >
		<div class="container">
			<center>
				...
				<table>
					...
					<br>
					<tr>
						<td style="color: white; font-family:arial,sans-serif; -webkit-font-smoothing:antialiased; font-size:120%;">Number of shirts given : '.$num_cd.'</td>
					</tr>
				</table>
			</center>
		</div>
